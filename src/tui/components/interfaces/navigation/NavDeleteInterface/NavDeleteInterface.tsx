...
interface NavDeleteInterfaceProps {
  tree: NavigationTree;
  instance: string;
  onClose: () => void;
  onSuccess: () => void;
...
export function NavDeleteInterface({
  tree,
  instance,
  onClose,
  onSuccess,
  onStatusChange,
}: NavDeleteInterfaceProps) {
  ...
  const footerHelpText = useMemo(() => {
    ...
    if (currentTab === "select") {
      if (inSelectMode) {
        return `${baseHelp} • ↑↓ navigate • → expand • ← collapse • Space toggle • Esc back`;
      }
      return `${baseHelp} • ↓ enter select mode`;
    } else if (currentTab === "review") {
      if (inReviewMode) {
        return `${baseHelp} • ↑↓ navigate buttons • Enter confirm • Esc back`;
      }
  ...
  const headerTitle = useMemo(() => {
    return currentTab === "select"
      ? "Delete Navigation - Select Items"
      : "Delete Navigation - Review & Confirm";
  }, [currentTab]);

  useFooterHelp(footerHelpText);
  useHeaderData({ title: headerTitle, metadata: `(${tree.locale})` });
